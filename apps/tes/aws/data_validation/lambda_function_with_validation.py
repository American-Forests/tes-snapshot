...
from data_validation.validation_integration import initialize_validation
...
def handler(event, context):
    ...
    logger.info(f"Database URL secret name: {database_url_secret_name}")
    ...
    logger.info(f"Checking if {key} is a National Explorer, US TESA, or Toronto TESA upload...")
    ...
    logger.info(f"File set: {file_set}")
    ...
    if file_set == 'national' or file_set == 'toronto-tesa':
        ...
        for file in required_files[file_set]:
            ...
            logger.info(f"Checking if {file_key} exists...")
            try:
    ...
    if file_set == 'national':
        ...
        for file in required_files[file_set]:
            ...
            logger.info(f"Downloading {file_key} to {download_path}...")
            s3.download_file(input_bucket, file_key, download_path)
        
        logger.info(f"All required national files downloaded to {temp_dir}")
        ...
        for file in required_files[file_set]:
            ...
            logger.info(f"Downloading {file_key} to {download_path}...")
            s3.download_file(input_bucket, file_key, download_path)
        logger.info(f"All required toronto files downloaded to {temp_dir}")
    ...
    logger.info("File(s) loaded successfully")
